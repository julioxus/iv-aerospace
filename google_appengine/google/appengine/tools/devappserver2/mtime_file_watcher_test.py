...
class TestMtimeFileWatcher(unittest.TestCase):
  ...
  def test_file_created(self):
    self._watcher.start()
    self._watcher._changed_paths()
    path = self._create_file('test')
    self.assertEqual(self._watcher._changed_paths(), {path})
  ...
  def test_file_read(self):
    ...
    self.assertEqual(self._watcher._changed_paths(), set())
  ...
  def test_file_renamed(self):
    ...
    self.assertEqual(self._watcher._changed_paths(), {source, target})
  ...
  def test_file_created_in_directory(self):
    ...
    self.assertEqual(self._watcher._changed_paths(), {dir_path, path})
  ...
  def test_move_directory_out_of_watched(self):
    ...
    self._watcher._changed_paths()
    os.rename(source, target)
    self.assertEqual(self._watcher._changed_paths(), {source})
  ...
  def test_move_directory_into_watched(self):
    ...
    self._watcher._changed_paths()
    os.rename(source, target)
    self.assertEqual(self._watcher._changed_paths(), {target})
    file_path = os.path.join(target, 'file')
    with open(file_path, 'w+'):
      pass
    self.assertEqual(self._watcher._changed_paths(), {target, file_path})
  ...
  def test_symlink(self):
    ...
    self.assertEqual(
        self._watcher._changed_paths(), {sym_target, sym_file_path})

    # Check that a removed symlinked directory is reported.
    os.remove(sym_target)
    self.assertEqual(
        self._watcher._changed_paths(),
        {sym_target, sym_file_path, sym_subdir_path})
  ...
  def test_too_many_files(self):
    ...
    self.assertEqual(len(self._watcher._changed_paths()), 10000)
